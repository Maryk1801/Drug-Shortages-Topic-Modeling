...
setwd("/Users/maryannakay/Desktop/test2")
...
# tweets file
drugs1 <- read.csv("drugs shortage 180521.csv")
...
termsByTopic <- terms(lda8, 30)
termsByTopic
posterior(lda8)$topics[1:30,] #topic distribution for each document (ten first documents)
...
ap_top_terms %>%
  ...
  labs(title ="Topic Modeling- Top Terms By Topic (betas) For 10-17 May 2021") + 
  scale_y_reordered()
...
ggplot(topics.by.date, aes(date, fill = topics.labeled)) + geom_density(alpha = 0.5) +   # alpha is a transparency param
  scale_fill_manual(values = c( "#F06292", "#7B1FA2", "#80DEEA", "#AED581", "#FFF176", "#FFB74D", "#A1887F", "#212121")) +
  labs(title ="Density Plot For 10-17 May 2021") 
...
ggplot(topics.by.date, aes(date, fill = topics.labeled)) + 
  ...
  scale_fill_manual(values = c( "#F06292", "#7B1FA2", "#80DEEA", "#AED581", "#FFF176", "#FFB74D", "#A1887F", "#212121")) +
  labs(title ="Bar Plot For 10-17 May 2021") 
