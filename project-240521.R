...
setwd("/Users/maryannakay/Desktop/240521")
...
# tweets file
drugs1 <- read.csv("drugs shortage 240521.csv")
...
termsByTopic <- terms(lda9, 10)
termsByTopic
posterior(lda9)$topics[1:10,] #topic distribution for each document (ten first documents)


topics <- topics(lda9)
topics.labeled <- recode(topics, '1'='Topic 1','2'='Topic 2','3'='Topic 3','4'='Topic 4', '5'='Topic 5','6'='Topic 6', '7'='Topic 7', '8'='Topic 8', '9'='Topic 9')
table(topics.labeled) #number of terms in each topic 
...
ap_top_terms %>%
  ...
  labs(title ="Topic Modeling- Top Terms By Topic (betas) For May 16th-23rd") + 
  scale_y_reordered()
...
ggplot(topics.by.date, aes(date, fill = topics.labeled)) + geom_density(alpha = 0.5) +   # alpha is a transparency param
  scale_fill_manual(values = c("#F44336", "#F06292", "#7B1FA2", "#80DEEA",  "#AED581", "#FFF176", "#FFB74D", "#A1887F", "#212121")) +
  labs(title ="Density Plot For May 16th-23rd") 
...
ggplot(topics.by.date, aes(date, fill = topics.labeled)) + 
  ...
  scale_fill_manual(values = c("#F44336", "#F06292", "#7B1FA2", "#80DEEA",  "#AED581", "#FFF176", "#FFB74D", "#A1887F", "#212121")) +
  labs(title ="Bar Plot For May 16th-23rd") 
