...
setwd("/Users/maryannakay/Desktop/test")
...
drugs2 <- read.csv("drugs shortage 240521.csv")
drugs.all<-rbind(drugs1,drugs2)
#create df of tweets
...
#LDA-8-----------------------------
lda8 <- LDA(dtm, k = 8, control=list(seed=77,alpha=0.1),method = "Gibbs") #lower alpha, we assume documents contain fewer topics
termsByTopic <- terms(lda8, 10)
termsByTopic
posterior(lda8)$topics[1:10,] #topic distribution for each document (ten first documents)

topics <- topics(lda8)
topics.labeled <- recode(topics, '1'='Topic 1','2'='Topic 2','3'='Topic 3','4'='Topic 4', '5'='Topic 5','6'='Topic 6', '7'='Topic 7', '8'='Topic 8')
table(topics) #number of terms in each topic 

colnames(termsByTopic) <- c('1'='Topic 1','2'='Topic 2','3'='Topic 3','4'='Topic 4', '5'='Topic 5','6'='Topic 6', '7'='Topic 7', '8'='Topic 8')


#most common term within each topic
ap_topics<- tidy(lda8,matrix="beta")
...
ap_top_terms %>%
  ...
  scale_fill_manual(values = c( "#F06292", "#7B1FA2", "#80DEEA", "#AED581", "#FFF176", "#FFB74D", "#A1887F", "#212121")
                    , name = "Topics", labels = c("1","2","3","4","5","6", "7", "8", "9")) +
  facet_wrap(~topic, scales="free") + 
  labs(title ="Topic Modeling- Top Terms By Topic (betas) For All Dates") + 
  scale_y_reordered()
...
ggplot(topics.by.date, aes(date, fill = topics.labeled)) + geom_density(alpha = 0.5) +   # alpha is a transparency param
  scale_fill_manual(values = c( "#F06292", "#7B1FA2", "#80DEEA", "#AED581", "#FFF176", "#FFB74D", "#A1887F", "#212121")) +
  labs(title ="Density Plot For All Dates") 
...
ggplot(topics.by.date, aes(date, fill = topics.labeled)) + 
  ...
  scale_fill_manual(values = c( "#F06292", "#7B1FA2", "#80DEEA", "#AED581", "#FFF176", "#FFB74D", "#A1887F", "#212121")) +
  labs(title ="Bar Plot For All Dates") 
